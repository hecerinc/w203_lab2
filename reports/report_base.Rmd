...
## Import raw data
cpi <- read.csv('data/raw/cpi_fnb.csv', header=T)                 # Consumer Price Index
unemployment <- read.csv('data/raw/unemployment.csv', header=T)   # Unemployment Rate
approval <- read.csv('data/raw/approval_biden_538.csv', header=T) # Approval Rating
gas <- read.csv('data/raw/wti.csv')                               # Crude Oil Prices: West Texas Intermediate (WTI)
covid <- read.csv('data/raw/United_States_COVID-19_Cases_and_Deaths_by_State_over_Time.csv', header=T)
```
...
## Approval
print(range(approval$modeldate)) # we got from 2021-01-23 to 2021-12-03

## Gas
filter(gas, month(date) == 10, year(date)==2021) %>% arrange(date)

## COVID cases (total + new)
covid <- covid %>%
  rename(date = submission_date) %>%
  mutate(date=as.Date(date, "%m/%d/%Y")) %>%
  group_by(date) %>%
  summarise(total_covid_cases = sum(tot_cases), new_covid_cases = sum(new_case))
...
approval <- approval %>%
  filter(date >= '2021-01-23', subgroup == "All polls") %>%
  rename(approval_rating = approve_estimate) %>%
  select(president, date, approval_rating)
gas <- gas %>%
  filter(date >= '2021-01-22') %>%                    # We need gas price for 1/23-24 which bases its value from 1/22
  rename(gas = dcoilwtico)
unemployment <- unemployment %>% 
  filter(date >= '2021-01-01')               # This is a monthly data (we need to include data for Jan. 2021)
cpi <- cpi %>% 
  filter(date >= '2021-01-01') %>%                    # This is a montnly data (we need to include data for Jan. 2021)
  rename(cpi = cpifabsl)
```

```{r include=FALSE}
# For each missing date, replace the missing value with the previous value
ts <- seq(ymd("2021-01-22"), ymd("2021-12-03"), by="day")  # 2021-01-22 has the value to replace for 1/23-24
...
tt <- data.table(gas)
setnafill(tt, type="locf", cols='gas')
gas <- data.frame(tt)
...
# Merge gas + COVID
gas.cov <- inner_join(gas, covid, by='date')

# Create a single dataframe of the all 3 independent variables - gas, COVID cases, cpi, and unemployment
# Since unemployment is monthly data, we explode it to daily sequence 
# with same value for each day in the respective month.
combined_indvars <- full_join(gas.cov, unemp_join_cpi, by="year_month", all = TRUE) %>%
  select(-year_month)

## Combine Y with Xs
df <- inner_join(approval, combined_indvars, by = 'date')  # Full data
```
...
unr.ap <- df %>%
  ...
  geom_line(aes(y = approval_rating/uc_coef, color = 'approval_rating')) +
  scale_y_continuous(
    ...
    sec.axis = sec_axis(~.*uc_coef, name="approval_rating")
  ) +
  scale_color_manual(name = "variables", values = c("unrate" = "red", "approval_rating" = "darkorange")) +
  labs(
...
ar.g <- df %>%
  ggplot() +
  aes(x = date) +
  geom_line(aes(y = approval_rating, color = 'approval_rating')) +
  ...
  scale_color_manual(name = "variables", values = c("approval_rating" = "darkblue", "gas" = "darkred")) +
  labs(
...
cpi.ap <- df %>%
  ...
  geom_line(aes(y = approval_rating/ca_coef, color = 'approval_rating')) +
  scale_y_continuous(
    ...
    # Add a second axis and specify its features
    sec.axis = sec_axis(~.*ca_coef, name="approval_rating")
  ) +
  scale_color_manual(name = "variables", values = c("cpi" = "darkorange", "approval_rating" = "darkblue")) +
  labs(
