...
load('../data/interim/df_with_events.RData') # TODO: change this to /processed
```
...
Events are the points in the timeline which have a special effect on our outcome variables. We observed that in every presidency there are these events which correlate with fluctuations in the approval rating. Our theory is that the effect stays until the topic is matter or concern or discussion warm in the news cycle. To account for these in our model, we first identified a bunch of these events across the tenute all four presidents we are modeling with and we added an indicator variable based on the time following the event.
...
We won't be able to accurately calculate the effect because of unavailability of accurate and measurable data on `natural_disaster`, however we will try to speculate the effect of coefficient and direction of bias with the other covariates based on our best judgement.
